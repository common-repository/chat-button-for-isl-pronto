...
if ( ! class_exists( 'DD_ISLPronto' ) ) {
	...
	class DD_ISLPronto {
		...
		private function render_chat( $attrs ) {
			...
			// If the position isn't static or if stay on top is set, be lazy and build an inline style statement.
			...
			if ( $attrs['stayontop'] ) {
				if ( ! empty( $position_style ) ) {
					$position_style = substr( $position_style, 0, -1 ) . '; z-index: 999;';
				} else {
					$position_style = 'z-index: 999;';
				}
			}
			
			$position_style = apply_filters( 'dd_isl_pronto_chat_style', $position_style );
			$position_class = apply_filters( 'dd_isl_pronto_chat_class', 'dd-isl-pronto-button' );
			
			// Render the chat button.
			?>
			<a href="<?php echo esc_url( $attrs['offlineurl'] ); ?>" id="islpronto_link" style="<?php echo esc_attr( $position_style ); ?>" class="<?php echo esc_attr( $position_class ); ?>">
			  <img alt="Live chat" id="islpronto_image" src="<?php echo esc_url( $attrs['imagepath'] ); ?>/islpronto-message.jpg" style="border:none" />
			</a>
			<?php 
		...
		public function render_shortcode( $attrs = null ) {
			...
			$my_atts = shortcode_atts(
				array(
					...
					'stayontop'  => get_option( 'dd_isl_pronto_stayontop',  self::DEFAULTS['dd_isl_pronto_stayontop'] ),
				),
				$attrs
			);
		...
		public function register_settings() {
			...
			register_setting( 'dd-isl-pronto', 'dd_isl_pronto_stayontop' );
		}
		...
		public function build_settings() {
			...
			add_settings_field( 'dd_isl_pronto_stayontop',  __( 'Keep button on top of other elements?','dd-isl-pronto' ), array( $this, 'render_stayontop' ), 'dd-isl-pronto', 'dd_isl_pronto' );
		}
		...
		public function render_stayontop() {  $this->render_checkbox( 'dd_isl_pronto_stayontop' ); }
